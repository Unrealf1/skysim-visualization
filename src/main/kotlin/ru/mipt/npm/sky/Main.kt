...
import ru.mipt.npm.sky.visualization.Generation
import java.io.File


const val programmName = ""
    ...
    fun start(channel: Channel<Generation>) {
        runBlocking {
            ...
            flow.onEach { generation ->
                ...
                val visualGen = Generation(
                    generation.map {photon->
                        Point3D(
                            photon.origin.x,
                            photon.origin.y,
                            photon.origin.z
                ...
                channel.send(visualGen)
                i++
            }
            channel.send(Generation(emptyList()))
        }
